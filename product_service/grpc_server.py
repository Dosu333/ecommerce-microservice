...
class ProductService(product_pb2_grpc.ProductServiceServicer):
    def CheckStock(self, request, context):
        ...
        if product:
            return product_pb2.ProductResponse(available=product["stock"] > request.quantity, price=product["price"], vendor=product["vendor_id"], name=product['name'])
        return product_pb2.ProductResponse(available=False, price=0.0, vendor="", name="")
